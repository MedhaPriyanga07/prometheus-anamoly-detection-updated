...
if __name__ == "__main__":
    ...
    # Option A: Run both Tornado and schedule in the **same** thread (blocking).
    # A typical pattern is to integrate schedule inside the Tornado ioloop 
    # with a PeriodicCallback, but for simplicity, you can just do a 
    # separate while loop in a new thread.
    ...
    # Start schedule loop on a background thread
    t = Thread(target=schedule_loop, daemon=True)
    t.start()

    # Start Tornado on the main thread
